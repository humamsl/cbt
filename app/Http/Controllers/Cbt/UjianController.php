...
class UjianController extends Controller
{
     ...
     */
    public function logViolation(Quiz $quiz, QuizAttempt $attempt, Request $r, \App\Services\Ujian\ExamScoringService $scoring)
    {
        abort_unless($attempt->siswa_id === $r->user()->id, 403);
        ...
        if ($count >= $threshold) {
            switch ($mode) {
                case 'logout_otomatis':
                    ...
                    $scoring->finalize($quiz, $attempt, forced: true);
                    $shouldLogout = true;
                    break;
                case 'blokir':
                    $scoring->blockAndFinalize($quiz, $attempt, 'Mencapai batas '.$threshold.' pelanggaran (terakhir: '.$data['type'].')');
                    $shouldBlock = true;
                    break;
                case 'peringatan':
    ...
    public function submit(Quiz $quiz, QuizAttempt $attempt, Request $r, \App\Services\Ujian\ExamScoringService $scoring)
    {
        abort_unless($attempt->siswa_id === $r->user()->id, 403);
        if ($attempt->is_blocked) {
            return redirect()->route('siswa.ujian.blocked', [$quiz, $attempt]);
